...
import com.example.wuhongjie.myapplication.type.Update;
...
public class UpdateManager {
		...
		new Thread(){
			public void run() {
				...
				try {
					...
					String ret= WebUtils.doGet("http://URL/YoungorTgOrder/getLastVersion.do",
					...
					if(dataJson.length()>0){
						...
						update.setVersionCode(Integer.parseInt(jsonobject.getString("VER_CODE")));
						update.setVersionName(jsonobject.getString("VER_NUM"));
						update.setDownloadUrl(jsonobject.getString("URL"));
						update.setUpdateLog(jsonobject.getString("REMARKS"));
	...
	private Runnable mdownApkRunnable = new Runnable() {	
		...
		public void run() {
			try {
				String apkName = "TGApp_"+mUpdate.getVersionName()+".apk";
				String tmpApk = "TGApp_"+mUpdate.getVersionName()+".tmp";
				...
				if(storageState.equals(Environment.MEDIA_MOUNTED)){
					savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/YoungorTgCache/";
